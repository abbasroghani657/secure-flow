...
export default function Blog() {
  ...
  return (
    <div>
      <MarketingNav />
      <header style={{ maxWidth: 900, margin: "0 auto", padding: "72px 28px 20px", textAlign: "center" }}>
      ...
      <section style={{ maxWidth: 1120, margin: "0 auto", padding: "40px 28px 20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 20 }}>
          {rest.map((p) => (
            <Link key={p.slug} to={`/blog/${p.slug}`} style={{ display: "flex", flexDirection: "column", borderRadius: 16, border: `1px solid ${T.border}`, background: "rgba(255,255,255,0.02)", padding: "26px 24px", gap: 12 }} className="blog-card">
