...
const UPLOAD_TYPES = ["mobile", "sca", "ios", "iac", "secrets", "cicd", "sast", "api", "container"];
            ...
            {UPLOAD_TYPES.includes(type) ? (
              ...
              (() => {
                const cfg = {
                  ...
                  sca: { label: "Dependency file", file: depFile, set: setDepFile, accept: ".json,.txt,.lock,.mod,.sum,.xml,.gradle,.kts,.csproj,.config,.props", prompt: "package.json / requirements.txt / pom.xml / build.gradle / *.csproj / lock file", note: "Your dependencies (npm, PyPI, Go, Ruby, PHP, Rust, Maven/Gradle, NuGet) are checked against the OSV vulnerability database, then the file is deleted." },
                  ...
                  container: { label: "Container image (docker save tar)", file: containerFile, set: setContainerFile, accept: ".tar", prompt: "Click to choose an image .tar (docker save)", note: "OS-package CVEs (Debian/Ubuntu/Alpine via OSV), secrets baked into layers, and config issues; then the file is deleted." },
                }[type];
                const f = cfg.file;
                return (
