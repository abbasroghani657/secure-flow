...
_JS_RULES = [
    R("sast-js-code-injection", "Code injection via eval()/Function()", "high",
      r"\b(eval|new\s+Function)\s*\(",
      "eval()/new Function() executes a string as code.",
    ...
    R("sast-js-command-injection", "OS command injection (child_process)", "high",
      r"child_process\.(exec|execSync)\s*\(\s*[`'\"]?[^)]*\$\{|\.exec(Sync)?\s*\([^)]*\+",
    ...
    R("sast-js-dom-xss", "DOM XSS sink (innerHTML/document.write)", "medium",
      r"\.(innerHTML|outerHTML)\s*=|\bdocument\.write\s*\(|dangerouslySetInnerHTML",
    ...
    R("sast-js-weak-random", "Math.random() used for security value", "low",
      r"(token|secret|key|password|nonce|otp|session)\w*\s*=\s*[^;\n]*Math\.random\s*\(",
      "Math.random() is not cryptographically secure.",
      "Use crypto.randomBytes()/crypto.getRandomValues() for security tokens."),
]
